...
#include "Suffix_Array_Modified.hpp"
...
namespace CaPS_SA
{
    ...
    Suffix_Array<T_idx_>::Suffix_Array(const char *const T, const idx_t n, const idx_t subproblem_count, const idx_t max_context) : T_(T),
                                                                                                                                    n_(n),
    ...
    void Suffix_Array<T_idx_>::merge(const idx_t *X, idx_t len_x, const idx_t *Y, idx_t len_y, const idx_t *LCP_x, const idx_t *LCP_y, idx_t *Z, idx_t *LCP_z) const
    {
        ...
        while (i < len_x && j < len_y)
        {
            ...
            else // Compute LCP of X_i and Y_j through linear scan.
            {
                ...
                const idx_t n = m + LCP(T_, T_ + (X[i] + m), T_ + (Y[j] + m), context - m); // LCP(X_i, Y_j)
    ...
    void Suffix_Array<T_idx_>::merge_sort(idx_t *const X, idx_t *const Y, const idx_t n, idx_t *const LCP, idx_t *const W) const
    {
        ...
        else
        {
            ...
            m < nested_par_grain_size ? (f(), g()) : parlay::par_do(f, g);
    ...
    {
        std::cerr << "Starting to locate pivots.\n";
    ...
    T_idx_ Suffix_Array<T_idx_>::upper_bound(const idx_t *const X, const idx_t n, const char *const P, const idx_t P_len) const
    {
        ...
        while (r - l > 1) // Candidate matches exist.
        {
            ...
            lcp_c += LCP(T_, suf + lcp_c, P + lcp_c, max_lcp - lcp_c); // Skip an informed number of character comparisons.
    ...
    void Suffix_Array<T_idx_>::sort_partition(idx_t *const X, idx_t *const Y, const idx_t n, const idx_t *const S, idx_t *const LCP_x, idx_t *const LCP_y)
    {
        ...
        (flat_count_l < nested_par_grain_size || flat_count_r < nested_par_grain_size) ? (f(), g()) : parlay::par_do(f, g);
    ...
    void Suffix_Array<T_idx_>::compute_partition_boundary_lcp()
    {
            ...
            [&](const idx_t j)
        {
            ...
            LCP_[part_idx] = LCP(T_, T_ + SA_[part_idx - 1], T_ + SA_[part_idx], n_ - std::max(SA_[part_idx - 1], SA_[part_idx]));
        };
    ...
    void Suffix_Array<T_idx_>::construct()
    {
        ...
        std::cerr << "Cache Lookups: " << Suffix_Array<idx_t>::cache_lookups.load() << "\n";
        std::cerr << "Cache Hits: " << Suffix_Array<idx_t>::cache_hits.load() << "\n";
        std::cerr << "Cache Hit Ratio: "
                  << Suffix_Array<idx_t>::cache_hit_ratio()*100.0 << "\n";
    }
